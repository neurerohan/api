...
async def scrape_panchang(db: Session) -> Dict:
    ...
    """
    try:
        # Using Ashesh.com.np panchang widget
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Extract Nepali date parts
            date_div = soup.select_one(".info_wrap")
            
            if not date_div:
                logger.warning("Could not find date information on the page")
                return {}
                
            # Extract date information
            today_info = {}
            
            # Find the parts of the date
            nepali_date_elem = soup.select_one(".nepali_date")
            english_date_elem = soup.select_one(".english_date")
            tithi_elem = soup.select_one(".tithi")
            
            # Additional elements
            month_info_elem = soup.select_one(".monthinfo")
            event_elem = soup.select_one(".event")
            
            # Sun and moon information
            sunrise_elem = soup.select_one(".sunrise")
            sunset_elem = soup.select_one(".sunset")
            moonrise_elem = soup.select_one(".moonrise")
            moonset_elem = soup.select_one(".moonset")
            
            # Extract the text content of each element
            nepali_date = nepali_date_elem.text.strip() if nepali_date_elem else ""
            english_date = english_date_elem.text.strip() if english_date_elem else ""
            tithi = tithi_elem.text.strip() if tithi_elem else ""
            month_info = month_info_elem.text.strip() if month_info_elem else ""
            event = event_elem.text.strip() if event_elem else ""
            
            # Parse date strings
            nepali_date_parts = nepali_date.split(" ")
            nepali_weekday = nepali_date_parts[0].replace(",", "") if len(nepali_date_parts) > 0 else ""
            nepali_day = nepali_date_parts[1] if len(nepali_date_parts) > 1 else ""
            nepali_month = nepali_date_parts[2] if len(nepali_date_parts) > 2 else ""
            nepali_year = nepali_date_parts[3] if len(nepali_date_parts) > 3 else ""
            
            english_date_parts = english_date.split(" ")
            english_weekday = english_date_parts[0].replace(",", "") if len(english_date_parts) > 0 else ""
            english_day = english_date_parts[1] if len(english_date_parts) > 1 else ""
            english_month = english_date_parts[2] if len(english_date_parts) > 2 else ""
            english_year = english_date_parts[3] if len(english_date_parts) > 3 else ""
            
            # Extract month number from Nepali month
            month_num = 0
            ...
            # Extract sun and moon information
            sunrise = sunrise_elem.text.strip() if sunrise_elem else ""
            sunset = sunset_elem.text.strip() if sunset_elem else ""
            moonrise = moonrise_elem.text.strip() if moonrise_elem else ""
            moonset = moonset_elem.text.strip() if moonset_elem else ""
            
            # Format sun/moon info for easier access
            sun_moon_formatted = {
                "sunrise": sunrise,
                "sunset": sunset,
                "moonrise": moonrise,
            ...
            # Construct return object with all information
            today_info = {
...
async def scrape_calendar(db: Session, year: int = None, month: int = None) -> List[Dict]:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Process the calendar cells
            try:
                for i in range(0, len(day_cells), 7):
                    for j in range(7):
                        if i + j >= len(day_cells):
                            continue
                ...
                logger.error(f"Error processing calendar cells: {str(e)}")
